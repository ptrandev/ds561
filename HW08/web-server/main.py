...
def get_file(path):
  ...
  if country and country.lower() in banned_countries:
    ...
    return 'Banned country', 400, {'X-GCP-ZONE': GCP_ZONE}

  # only accept GET method
  if request.method != 'GET':
    logging_client.log_text(f'Method not implemented: {request.method}')
    return 'Method not implemented', 501, {'X-GCP-ZONE': GCP_ZONE}
  ...
  if file_name is None:
    ...
    return 'file_name is required', 400, {'X-GCP-ZONE': GCP_ZONE}
  
  if bucket_name is None:
    print('bucket_name is required')
    return 'bucket_name is required', 400, {'X-GCP-ZONE': GCP_ZONE}
  ...
  if blob.exists():
    ...
    return blob_content, 200, {'Content-Type': 'text/html; charset=utf-8', 'X-GCP-ZONE': GCP_ZONE}

  logging_client.log_text(f'File not found: {bucket_name}/{file_name}')
  return 'File not found', 404, {'X-GCP-ZONE': GCP_ZONE}
