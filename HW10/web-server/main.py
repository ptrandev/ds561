...
with pool.connect() as conn:
    ...
    conn.execute(
        sqlalchemy.text(
            """
            CREATE TABLE IF NOT EXISTS fail_request (
                fail_request_id INT AUTO_INCREMENT PRIMARY KEY,
                time_of_request TIMESTAMP,
                requested_file VARCHAR(255),
                error_code INT
            );
...
def publish_fail_request_to_database(time_of_request, requested_file, error_code):
    with pool.connect() as conn:
        conn.execute(
            sqlalchemy.text(
                """
